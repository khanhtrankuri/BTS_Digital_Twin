...
)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			const float grad_mean_x = dL_dG * dG_ddelx * ddelx_dx;
			const float grad_mean_y = dL_dG * dG_ddely * ddely_dy;
			atomicAdd(&dL_dmean2D[global_id].x, grad_mean_x);
			atomicAdd(&dL_dmean2D[global_id].y, grad_mean_y);
			// AbsGS statistic only: preserve the signed gradient above for autograd.
			atomicAdd(&dL_dmean2D_abs[global_id].x, fabsf(grad_mean_x));
			atomicAdd(&dL_dmean2D_abs[global_id].y, fabsf(grad_mean_y));
